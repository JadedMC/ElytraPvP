...
/**
 * Runs the Trail Selector GUI.
 */
public class TrailsGUI extends CustomGUI {

    /**
     * Shows the Trail GUI.
     * This lists each trail.
     * @param plugin Instance of the plugin.
     * @param player Player to get GUI of.
     * @param page Page number of the GUI.
     */
    public TrailsGUI(ElytraPvP plugin, Player player, int page) {
        ...
        ArrayList<Trail> trails = new ArrayList<>(plugin.cosmeticManager().getTrails());

        int s = 0;
        for(int i = ((page - 1) * 21) + 1; i <= trails.size(); i++) {
            if(i > (page * slots.length) || s > slots.length - 1) {
                break;
            }

            Trail trail = trails.get(i - 1);
            setItem(slots[s], trail.getIcon(player), (p, a) -> {
                CustomPlayer customPlayer = plugin.customPlayerManager().getPlayer(player);

                // Sets the chat if the player has it already unlocked.
                if(customPlayer.getUnlockedTrails().contains(trail.getId())) {
                    customPlayer.setTrail(trail);
                    player.closeInventory();
                    ChatUtils.chat(player, "&a&lCosmetics &8» &aTrail has been equipped.");
                    return;
                }

                // Otherwise, checks the unlock requirements.
                switch (trail.getUnlockType()) {
                    ...
                    case NORMAL -> {
                        ...
                        if(plugin.seasonManager().getCurrentSeason() != trail.getSeason() && trail.getSeason() != Season.NONE) {
                            return;
                        }

                        // Make sure the player has enough coins.
                        if(customPlayer.getCoins() < trail.getPrice()) {
                            ChatUtils.chat(p, "&c&lError &8» &cYou do not have enough coins for that.");
                            return;
                        }

                        // Unlocks and equips the hat.
                        customPlayer.removeCoins(trail.getPrice());
                        customPlayer.unlockTrail(trail);
                        customPlayer.setTrail(trail);
                        player.closeInventory();
                        ChatUtils.chat(player, "&a&lCosmetics &8» &aTrail has been purchased and equipped.");
        ...
        if(page == 1) {
            ...
            setItem(38, new SkullBuilder("eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvODU1MGI3Zjc0ZTllZDc2MzNhYTI3NGVhMzBjYzNkMmU4N2FiYjM2ZDRkMWY0Y2E2MDhjZDQ0NTkwY2NlMGIifX19").setDisplayName("&aPage " + (page - 1)).build(), (p,a) -> new TrailsGUI(plugin, player, page - 1).open(player));
        }

        if(trails.size() > (page * 21)) {
            setItem(42, new SkullBuilder("eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvOTYzMzlmZjJlNTM0MmJhMThiZGM0OGE5OWNjYTY1ZDEyM2NlNzgxZDg3ODI3MmY5ZDk2NGVhZDNiOGFkMzcwIn19fQ==").setDisplayName("&aPage " + (page + 1)).build(), (p,a) -> new TrailsGUI(plugin, player, page + 1).open(player));
        }
        else {
        ...
        setItem(0, new SkullBuilder("eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvZjg0ZjU5NzEzMWJiZTI1ZGMwNThhZjg4OGNiMjk4MzFmNzk1OTliYzY3Yzk1YzgwMjkyNWNlNGFmYmEzMzJmYyJ9fX0=").setDisplayName("&cBack").build(), (p,a) -> new CosmeticsGUI(plugin, player).open(player));
        ...
        setItem(40, new ItemBuilder(Material.BARRIER).setDisplayName("&cReset").build(), (p, a ) -> {
            ...
            customPlayer.setTrail(null);
            p.closeInventory();
            ChatUtils.chat(p, "&a&lCosmetics &8» &aTrail has been reset.");
        });
    }
}